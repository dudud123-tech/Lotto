...
export class RouletteRenderer {
  ...
  protected _lottoBallImages: { [key: string]: CanvasImageSource } = {};
  ...
  private getMarbleImage(name: string): CanvasImageSource | undefined {
    const lottoNumber = Number(name);
    if (Number.isInteger(lottoNumber) && lottoNumber >= 1 && lottoNumber <= 45) {
      return this.getLottoBallImage(lottoNumber);
    }
  ...
  private getLottoBallImage(number: number): CanvasImageSource {
    const key = String(number);
    if (!this._lottoBallImages[key]) {
      this._lottoBallImages[key] = this.createLottoBallImage(number);
    }
    return this._lottoBallImages[key];
  }

  private getLottoPalette(number: number) {
    if (number <= 10) return { fill: '#f5a400', stroke: '#b77900' };
    if (number <= 20) return { fill: '#1976d2', stroke: '#0d47a1' };
    if (number <= 30) return { fill: '#d62f5f', stroke: '#9d174d' };
    if (number <= 40) return { fill: '#6b7280', stroke: '#374151' };
    return { fill: '#24a148', stroke: '#166534' };
  }

  private createLottoBallImage(number: number): CanvasImageSource {
    const palette = this.getLottoPalette(number);
    const size = 128;
    const ball = document.createElement('canvas');
    ball.width = size;
    ball.height = size;
    const ctx = ball.getContext('2d') as CanvasRenderingContext2D;
    const gradient = ctx.createRadialGradient(42, 34, 10, 64, 64, 58);
    gradient.addColorStop(0, '#fff');
    gradient.addColorStop(0.15, palette.fill);
    gradient.addColorStop(0.78, palette.fill);
    ...
    ctx.shadowBlur = 3;
    ctx.fillText(String(number), 64, 69);
    return ball;
  }
